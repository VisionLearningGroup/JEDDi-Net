...
def generate_roi(rois, rois_lstm, video, start, end, stride, split):
  tmp = {}
  wins = ( rois[:,:2] - start ) / stride
  
  sorted_inds = wins[:,1].argsort()
  
  tmp['wins'] = wins[sorted_inds,]
  tmp['input_sentence'] = rois_lstm[sorted_inds,0]
  tmp['cont_sentence'] = rois_lstm[sorted_inds,1]
  tmp['target_sentence'] = rois_lstm[sorted_inds,2]
...
cPickle.dump(train_roidb, open('train_data_modified_3fps_caption_768_sorted.pkl','w'), cPickle.HIGHEST_PROTOCOL)
